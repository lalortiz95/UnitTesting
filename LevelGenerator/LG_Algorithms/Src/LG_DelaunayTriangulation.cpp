...
namespace LevelGenerator
{
	...
	//! This function finds the triangle that we want to legalize it's edges.
	LG_Triangle * LG_DelaunayTriangulation::FindTriangleToLegalize(LG_Node * pFirstNode, LG_Node * pSecondNode, LG_Node * pThirdNode)
	{
		for (int32 i = 0; i < m_pTrianglesVector.size(); ++i)
		{
			if (m_pTrianglesVector[i]->CompareIndex(pFirstNode, pSecondNode, pThirdNode))
				return m_pTrianglesVector[i];
		}
		return nullptr;
	}
